...
redimensionner la tête de discriminateur, ce qu'on n'a pas implémenté. Côté
TabICL, ULTIMATE applique INLP sur \texttt{x} brut par simplicité ; or les
embeddings sont accessibles via \texttt{TabICLCache.row\_repr}. Validation
multi-seed $\times$ Pokec-z/n : INLP sur embeddings tombe le leakage gender
à \textbf{0.61-0.63} vs 0.71 sur \texttt{x} brut, avec reproduction
cross-dataset à $<$0.02 près
(\texttt{results/metrics/tabicl\_inlp\_embedding.csv}). Ré-injection dans le
prédicteur pour valider F1 retention : prochaine étape.
