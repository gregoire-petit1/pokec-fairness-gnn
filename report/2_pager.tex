...
\textbf{Outils d'IA} (mention exigée) : assistance algorithmique pour la
réimplémentation FairGNN-GRL, la migration pandas $\to$ polars,
l'intégration TabICL, l'ajout des modules INLP / calibration / reweighting,
et la rédaction. Code revu, testé, exécuté par les auteurs.

\textbf{Références.} Hardt-Price-Srebro 2016 ; Ravfogel et al.\ 2020 ;
Ganin \& Lempitsky 2015 ; Dai \& Wang 2021 ; Kamiran \& Calders 2012 ;
Chouldechova 2017 ; Crenshaw 1989 ; Hoffmann 2019 ; Hanna et al.\ 2020
(FAccT) ; Newman 2003 ; Qu et al.\ 2025 (TabICL) ; Laclau et al.\ 2024.

\newpage

\section*{Annexe — Tableaux détaillés}

\subsection*{A.1 Comparaison méthodes $\times$ axes (Pokec-z, seed=42)}

Source : \texttt{results/metrics/comparison\_full.csv}. Lecture : les
colonnes \textbf{Acc} et \textbf{F1} sont identiques par modèle (même
prédicteur sur tous les axes, c'est l'éval qui change). $\Delta$DP /
$\Delta$EO / Leakage varient par axe.
...
\textbf{GraphSAGE+ULTIMATE} & gender & 0.6155 & 0.5915 & 0.0090 & 0.0248 & \textbf{0.4996} \\
\textbf{TabICL+ULTIMATE} & gender & \textbf{0.8659} & \textbf{0.8657} & 0.0134 & 0.0008 & \textbf{0.5059} \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{A.2 ULTIMATE-LATENT vs ULTIMATE-x-brut — multi-seed [3, 7, 21, 42, 99]}

Source : \texttt{results/metrics/tabicl\_inlp\_reinjection\_composite.csv}.
ULTIMATE-LATENT projette dans \texttt{row\_repr} (512 dim) et ré-injecte
dans \texttt{icl\_predictor} ; ULTIMATE-x-brut applique INLP sur
\texttt{x} (264 dim) et utilise une LR séparée. \textbf{F1 par seed} :

\begin{center}\footnotesize
\begin{tabular}{l r r r}
\toprule
\textbf{Dataset} & \textbf{Seed} & \textbf{F1 baseline} & \textbf{F1 ULTIMATE-LATENT} \\
\midrule
...
pokec-n & 3 & 0.947 & \textbf{0.598} \\
pokec-n & 7 & 0.946 & 0.914 \\
pokec-n & 21 & 0.948 & 0.870 \\
pokec-n & 42 & 0.945 & \textbf{0.411} \\
pokec-n & 99 & 0.947 & \textbf{0.493} \\
\bottomrule
\end{tabular}
\hspace{1.5em}
\begin{tabular}{l r r}
\toprule
\textbf{Dataset} & \textbf{x-brut} & \textbf{LATENT ($\mu \pm \sigma$)} \\
\midrule
pokec-z & 0.866 & 0.884 $\pm$ 0.025 \\
pokec-n & 0.866 & \textbf{0.657 $\pm$ 0.214} \\
\bottomrule
\end{tabular}
\end{center}
